...
import { Table, Tag, Space, Button, Input, Select, Typography, Modal, message, App } from "antd"; // Import App from antd
import { ReloadOutlined, SearchOutlined, EditOutlined, DeleteOutlined, CheckOutlined, CloseOutlined, EyeOutlined } from "@ant-design/icons";
...
export default function AdminProductsPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: number; status: string }) => updateProduct(id, { status }),
    onSuccess: () => {
      message.success("Product status updated");
      queryClient.invalidateQueries({ queryKey: ["admin", "products"] });
    },
    onError: (error: any) => {
      message.error("Failed to update status: " + (error.response?.data?.error || error.message));
    }
  });
   ...
   const deleteProductMutation = useMutation({
    ...
    onError: (error: any) => {
        message.error("Failed to delete product: " + (error.response?.data?.error || error.message));
    }
  });
  ...
  const handleApprove = (product: Product) => {
    ...
    const isFuture = product.started_at && dayjs(product.started_at).isAfter(dayjs());
    const newStatus = isFuture ? "approved" : "active";
    
    modal.confirm({
      title: "Approve Product",
      content: `Are you sure you want to approve "${product.name}"? Status will be set to '${newStatus}'.`,
      onOk: () => updateStatusMutation.mutate({ id: product.product_id, status: newStatus }),
    });
  };
  ...
  const handleReject = (product: Product) => {
      modal.confirm({
        ...
        content: `Are you sure you want to reject "${product.name}"?`,
        okType: 'danger',
        onOk: () => updateStatusMutation.mutate({ id: product.product_id, status: "rejected" }), // Assuming 'rejected' or similar status exists, check schema? 'inactive'?
      });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: Product) => (
         <div className="flex flex-col">
             ...
             <span className="text-xs text-gray-400">Type: {record.product_type?.name}</span>
         </div>
      ),
    },
    {
        ...
        render: (_: any, record: Product) => {
            if (record.is_admin_shop) {
                return <Tag color="blue">Official Store</Tag>;
            }
            return (
                <div className="flex flex-col">
                    <span className="text-sm font-medium">{record.users?.shop?.shop_profile?.shop_name || record.users?.username}</span>
                    <span className="text-xs text-gray-500">{record.users?.first_name} {record.users?.last_name}</span>
                </div>
            )
        }
    },
    ...
    },
    {
      ...
      key: "created_at",
      render: (_: any, record: Product) => record.started_at ? dayjs(record.started_at).format("DD/MM/YYYY HH:mm") : "-",
      sorter: true,
    },
    {
      title: "Actions",
