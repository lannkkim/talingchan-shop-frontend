...
import { Table, Button, Modal, message, Tag, Checkbox, Input, Form } from "antd";
...
export default function RolesManagementPage() {
  ...
  const handleSavePerms = async () => {
    ...
    try {
      await adminService.updateRolePermissions(selectedRole.roles_id, checkedPerms);
  ...
  const columns = [
        ...
        render: (_: any, record: AdminRole) => (
            <div className="flex flex-wrap gap-1">
                {record.role_permissions.map((rp, idx) => (
                    rp.permissions ? <Tag key={idx} className="text-xs">{rp.permissions.name}</Tag> : null
                ))}
            </div>
        )
    },
    {
      ...
      key: "action",
      render: (_: any, record: AdminRole) => (
        <Button icon={<LockOutlined />} size="small" onClick={() => handleEditPerms(record)}>
          Permissions
        </Button>
      ),
    },
