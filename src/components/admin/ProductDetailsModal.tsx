    ...
    >
      ...
      <div className="mb-6">
        <Space className="w-full justify-between items-start">
           <div>
             <Title level={4} className="m-0 mb-1">{product.name}</Title>
             <Space separator={<div className="w-px h-3 bg-gray-300" />}>
               ...
               <Text type="secondary" className="text-xs">
                Created: {product.created_at ? dayjs(product.created_at).format("DD/MM/YYYY") : "-"}
               </Text>
             </Space>
           </div>
      ...
      <Row gutter={24}>
        ...
        <Col span={12} className="border-r border-gray-100">
          <div className="flex justify-between items-center mb-4">
             <Title level={5} className="m-0">Included Cards</Title>
             <Tag>{product.product_stock_card?.length || 0} items</Tag>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <Text strong className="block truncate text-sm" title={card?.name}>{card?.name}</Text>
        ...
        <Col span={12}>
          <Space orientation="vertical" size="middle" className="w-full">
            <div>
               ...
               <Text className="text-sm">{product.description || "No description provided."}</Text>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
               <Space orientation="vertical" className="w-full">
                    ...
                    {(() => {
                        ...
                        return activePrice ? (
                          <div className="text-right">
                             <Text strong className="text-lg text-blue-600 block">฿{Number(activePrice.price).toLocaleString()}</Text>
                             {activePrice.price_period_ended && (
                                <Text type="secondary" className="text-[10px] block">
                                   Valid until: {dayjs(activePrice.price_period_ended).format("DD/MM/YYYY HH:mm")}
                                </Text>
                             )}
                          </div>
                        ) : <Text>-</Text>;
                 ...
                 <div className="flex justify-between items-center">
                    <Text type="secondary">Total Cards</Text>
                    <Text strong>{product.product_stock_card?.reduce((sum, pc) => sum + pc.quantity, 0) || 0} cards</Text>
                 </div>
                 <div className="flex justify-between items-center">
                    <Text type="secondary">Start Date</Text>
                    <Text strong className="text-xs">{product.started_at ? dayjs(product.started_at).format("DD/MM/YYYY HH:mm") : "-"}</Text>
                 </div>
                 <div className="flex justify-between items-center">
                    <Text type="secondary">End Date</Text>
                    <Text strong className="text-xs">{product.ended_at ? dayjs(product.ended_at).format("DD/MM/YYYY HH:mm") : "-"}</Text>
                 </div>
               </Space>
            </div>
            ...
            <div>
              <Text type="secondary" className="block text-xs uppercase tracking-wider mb-3">Seller Information</Text>
              {product.is_admin_shop ? (
                <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                   ...
                   <div>
                     <Text strong className="block text-blue-900">Talingchan Official</Text>
                   </div>
                </div>
              ) : (
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                   <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold text-lg">
                     {product.users?.shop?.shop_profile?.shop_name?.charAt(0) || product.users?.username?.charAt(0) || "U"}
                   </div>
                   <div>
                     <Text strong className="block">{product.users?.shop?.shop_profile?.shop_name || product.users?.username || "Unknown"}</Text>
                     <Text type="secondary" className="text-xs">
